...
bool init()
{
    ...
    //Set up the screen
    SDL_VideoInfo v_info = *SDL_GetVideoInfo();
    SCREEN_WIDTH = v_info.current_w;
    SCREEN_HEIGHT = v_info.current_h;
...
bool Agent::handle_input()
{
    if( event.type == SDL_MOUSEBUTTONDOWN ) 
    {
        if (event.button.button == SDL_BUTTON_LEFT) {
            //std::cout << "mouse:\t(" << (camera.x + event.button.x);
            //std::cout << ", " << (camera.y + event.button.y) << ")\n";
            //std::cout << "dot:\t(" << box.x;
            //std::cout << ", " << box.y << ")\n";
            int dx = (camera.x + event.button.x) - (box.x + box.w);
            int dy = (camera.y + event.button.y) - (box.y + box.h);
            xVel = v_max*dx/sqrt(dx*dx+dy*dy);
            yVel = v_max*dy/sqrt(dx*dx+dy*dy);
        }
    }
