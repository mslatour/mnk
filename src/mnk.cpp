...
void Agent::handle_mouse_input()
{
  int mouseX, mouseY;
  if (SDL_GetMouseState(&mouseX, &mouseY)&SDL_BUTTON(1)) {
    int dx = (camera.x + mouseX) - (box.x + box.w/2);
    int dy = (camera.y + mouseY) - (box.y + box.h/2);
    int sq = (int) sqrt(dx*dx+dy*dy);
    int speed = (sq > v_max)? v_max: sq;
    if (sq == 0) {
      xVel = 0;
      yVel = 0;
    } else {
